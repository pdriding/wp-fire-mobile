...
export default function Header() {
    ...
    useEffect(() => {
      const handleScroll = throttle(() => {
        ...
        for (const id of ids) {
          ...
          if (visiblePercentage > 30 && rect.top <= 100) {
            activeSection = id;
            break;
          }
        }
        ...
        setActiveId(
          activeSection || (scrollPosition < viewportHeight * 0.3 ? "home" : "")
        );
      }, 100);
  ...
  const currentActive = useScrollSpy(sectionIds, 0);
  ...
  const handleHashClick = (e, hash) => {
    ...
    if (pathname === "/") {
      ...
      setMobileOpen(false); // close mobile menu when navigating away
    }
  };
